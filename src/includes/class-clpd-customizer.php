...
class Customizer {
    ...
    private function register_hooks() {
        ...
        $this->loader->add_filter('login_headerurl', $this, 'clpd_header_url');
        $this->loader->add_filter('login_headertext', $this, 'clpd_header_text');
    }
    ...
    public function get_custom_styles() {
        $background_type           = isset($this->settings['background_type']) ? esc_attr($this->settings['background_type']) : 'color';
        $background_color          = isset($this->settings['background_color']) ? esc_html($this->settings['background_color']) : '#f0f0f1';
        $background_gradient_start = isset($this->settings['background_gradient_start']) ? esc_html($this->settings['background_gradient_start']) : '#2271b1';
        $background_gradient_end   = isset($this->settings['background_gradient_end']) ? esc_html($this->settings['background_gradient_end']) : '#135e96';
        $background_image          = isset($this->settings['background_image']) ? esc_url_raw($this->settings['background_image']) : '';
        $logo_image                = isset($this->settings['logo_image']) ? esc_url_raw($this->settings['logo_image']) : '';
        $logo_image_width          = isset($this->settings['logo_image_width']) ? esc_html($this->settings['logo_image_width']) : '250px';
        $logo_image_height         = isset($this->settings['logo_image_height']) ? esc_html($this->settings['logo_image_height']) : '80px';
        $logo_image_radius         = isset($this->settings['logo_image_radius']) ? esc_html($this->settings['logo_image_radius']) : '0px';
        $text_color                = isset($this->settings['text_color']) ? esc_html($this->settings['text_color']) : '#3c434a';
        $text_font_family          = isset($this->settings['text_font_family']) ? esc_attr($this->settings['text_font_family']) : 'sans-serif';
        $button_text_color         = isset($this->settings['button_text_color']) ? esc_html($this->settings['button_text_color']) : '#ffffff';
        $button_bg_color           = isset($this->settings['button_bg_color']) ? esc_html($this->settings['button_bg_color']) : '#2271b1';
        $button_hover_bg_color     = isset($this->settings['button_hover_bg_color']) ? esc_html($this->settings['button_hover_bg_color']) : '#135e96';
        $button_font_family        = isset($this->settings['button_font_family']) ? esc_attr($this->settings['button_font_family']) : 'sans-serif';
        ...
        $custom_css .= 'font-family: "' . $text_font_family . '", sans-serif;';
        $custom_css .= '}';
        ...
        $custom_css .= 'font-family: "' . $button_font_family . '", sans-serif;';
     ...
     * @return string
     */
    public function clpd_header_url() {
        return home_url('/');
    }
     ...
     * @return string
     */
    public function clpd_header_text() {
